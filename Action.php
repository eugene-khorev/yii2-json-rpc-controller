...
class Action extends \yii\base\Action
{
    ...
    public function run()
    {
        ...
        try {
            ...
            if ($ex instanceof ValidationException) {
                $responseData['data'] = $ex->getData();
            } else {
                $responseData['data'] = YII_DEBUG ? $ex->getTraceAsString() : null;
            }
    ...
    protected function processRequest($requestData)
    {
        ...
        try {
            ...
            $responseData['error'] = [
                'code' => $ex->getCode(),
                'message' => $ex->getMessage(),
                'data' => ($ex instanceof ValidationException) 
                    ? $ex->getData()
                    : null,
            ];
        }
    ...
    protected function getRequestModel($requestData)
    {
        ...
        if (!$requestModel->validate()) {
            throw new ValidationException('Invalid JSON-RPC request parameters.', 0, $requestModel->getErrors());
        }
    ...
    protected function getMethodParams(\ReflectionMethod $reflectionMethod, $requestParams)
    {
        ...
        foreach ($methodParams as $param) {
            ...
            $actionParams[$paramName] = !empty($paramClass) ? $this->getParameterModel($paramName, $paramClass->name, $requestParams) : $requestParams[$paramName];
        }
    ...
    protected function getParameterModel($paramName, $modelClassName, $requestParams)
    {
        ...
        if (!$paramModel->validate()) {
            throw new ValidationException('Invalid JSON-RPC request parameters.', 0, $paramModel->getErrors());
        }
